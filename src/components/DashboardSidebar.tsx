...
import {
  ...
  HiSun, HiMoon, HiChevronLeft, HiChevronRight
} from 'react-icons/hi'
...
export default function DashboardSidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-page-light dark:bg-page-dark">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 ${collapsed ? 'w-20' : 'w-64'} bg-white dark:bg-slate-900/95 border-r border-slate-200 dark:border-white/10 transform transition-all duration-300 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 flex flex-col`}>
        <div className={`p-4 sm:p-6 border-b border-slate-200 dark:border-white/10 ${collapsed ? 'flex justify-center' : ''}`}>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-white/10">
          <button onClick={logout}
            className={`flex items-center gap-3 px-4 py-3 text-slate-500 dark:text-slate-400 hover:text-red-500 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 transition w-full font-medium ${collapsed ? 'justify-center px-0' : ''}`}>
            <HiLogout size={18} />
      ...
      <div className={`flex-1 flex flex-col min-h-screen max-w-full overflow-x-hidden transition-all duration-300 ml-0 ${collapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
        <header className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-white/10">
          <div className="flex items-center justify-between px-4 lg:px-8 h-16">
            <div className="flex items-center gap-2">
              ...
              <button className="hidden lg:flex text-slate-500 dark:text-slate-400 hover:text-gold-600 dark:hover:text-gold-400 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition" onClick={() => setCollapsed(!collapsed)} aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
